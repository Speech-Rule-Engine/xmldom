 ...
 */
export function LiveNodeList(node, refresh) {
	this._node = node;
 ...
 * @private
 */
export function _updateLiveList(list) {
...
Node.prototype = {
	...
	compareDocumentPosition: function (other) {
		...
		for (var n in ca.childNodes) {
			...
			if (chain2.includes(child)) return DocumentPosition.DOCUMENT_POSITION_FOLLOWING;
			if (chain1.includes(child)) return DocumentPosition.DOCUMENT_POSITION_PRECEDING;
		}
		return 0;
	},
};
 ...
 */
export function Document(symbol, options) {
	checkSymbol(symbol);
...
export function CharacterData(symbol) {
	checkSymbol(symbol);
}
...
export function Comment(symbol) {
	checkSymbol(symbol);
}
Comment.prototype = {
	nodeName: '#comment',
...
export function CDATASection(symbol) {
	checkSymbol(symbol);
}
...
export function DocumentType(symbol) {
	checkSymbol(symbol);
}
DocumentType.prototype.nodeType = DOCUMENT_TYPE_NODE;
_extends(DocumentType, Node);

export function Notation(symbol) {
	checkSymbol(symbol);
}
Notation.prototype.nodeType = NOTATION_NODE;
_extends(Notation, Node);

export function Entity(symbol) {
	checkSymbol(symbol);
}
Entity.prototype.nodeType = ENTITY_NODE;
_extends(Entity, Node);

export function EntityReference(symbol) {
	checkSymbol(symbol);
}
EntityReference.prototype.nodeType = ENTITY_REFERENCE_NODE;
_extends(EntityReference, Node);

export function DocumentFragment(symbol) {
	checkSymbol(symbol);
}
...
export function ProcessingInstruction(symbol) {
	checkSymbol(symbol);
}
...
// export const _updateLiveList = _updateLiveList;
// export const Attr = Attr;
// export const CDATASection = CDATASection;
...
// export const LiveNodeList = LiveNodeList;
// export const NamedNodeMap = NamedNodeMap;
